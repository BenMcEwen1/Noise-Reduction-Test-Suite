...
def train(training, validation, classes, num_classes, num_epochs=10, batch_size=1, early_stopping=True):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    BASE_MODEL_PATH = f"./classification/models/bird/resnet_model_denoised.pth"
...
def resnet(sr=16000):
    ...
    training = AudioDataset(ANNOTATIONS, AUDIO_DIR, mel_spectrogram, target_rate=sr, val=False)
    validation = AudioDataset(ANNOTATIONS, AUDIO_DIR, mel_spectrogram, target_rate=sr, val=True)
